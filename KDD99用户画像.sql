...
create table kdd99.balance as 
	...
	from kdd99.kdd99_card a
	left join kdd99.kdd99_disp b on a.disp_id=b.disp_id
	left join kdd99.kdd99_Trans c on b.account_id=c.account_id and a.issued>c.date
	order by a.card_id,c.date;
    
# 每张信用卡的最后交易日期
select * from kdd99.balance a
join (select card_id,max(trans_date) as max_date
       from kdd99.balance group by card_id) b 
on a.card_id=b.card_id and a.trans_date=b.max_date;

#2、开卡前的最后一次 交易间隔 和 账户余额
select a.card_id,c.date as trans_date,datediff(a.issued,c.date) as last_days,c.balance 
from kdd99.kdd99_card a
left join kdd99.kdd99_disp b on a.disp_id=b.disp_id
...
(select card_id,max(trans_date) as max_date
 from kdd99.balance group by card_id) d 
 on a.card_id=d.card_id and c.date=d.max_date
order by a.card_id;


###################################################################################################

#3、截止到开卡，交易笔数、半年内总入账、半年内总出账、半年内出/入账比例
-- "借"代表资金流出，"贷"代表资金流入
# 3.1 对kdd99.Trans进行数据清洗，将operation中的"支取现金"对应"借","贷"类型改为"借"，并将数据集保存到新表中
drop table trans_new;
...
sum(if(c.type='借',amount,0)) as trans_out_count,
sum(if(c.type='贷',amount,0)) as trans_in_count,
sum(if(c.type='借',amount,0))/avg(c.balance) as out_ratio,
sum(if(c.type='借',amount,0))/sum(if(c.type='贷',amount,0)) as out_in_ratio
  from kdd99.kdd99_card a
  left join kdd99.kdd99_disp b on a.disp_id=b.disp_id
...
select a.card_id,c.date as trans_date,datediff(a.issued,c.date) as last_days,c.balance from 
  kdd99.kdd99_card a
  left join kdd99.kdd99_disp b on a.disp_id=b.disp_id
  left join kdd99.kdd99_Trans c on b.account_id=c.account_id
  join (select card_id,max(trans_date) as max_date
...
select a.card_id,count(trans_id) as trans_count,
       sum(if(c.type='借',amount,0)) as trans_out_amount,
       sum(if(c.type='贷',amount,0)) as trans_in_amount,
       sum(if(c.type='借',amount,0))/avg(c.balance) as out_ratio,
       sum(if(c.type='借',amount,0))/sum(if(c.type='贷',amount,0)) as out_in_ratio
  from kdd99.kdd99_card a
  left join kdd99.kdd99_disp b on a.disp_id=b.disp_id
